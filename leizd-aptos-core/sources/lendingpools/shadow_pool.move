module leizd::shadow_pool {
    ...
    fun borrow_for_internal(
        ...
        let share = math64::to_share(amount_with_total_fee, asset_storage_ref.borrowed_amount, asset_storage_ref.borrowed_share);
        asset_storage_ref.borrowed_amount = asset_storage_ref.borrowed_amount + amount_with_total_fee;
        asset_storage_ref.borrowed_share = asset_storage_ref.borrowed_share + share;

        let borrowed_amount = &mut simple_map::borrow_mut<String, AssetStorage>(&mut storage_ref.asset_storages, &key).borrowed_amount;
        *borrowed_amount = *borrowed_amount + amount_with_total_fee;
    ...
    fun repay_internal(key: String, account: &signer, amount: u64): (u64, u64) acquires Pool, Storage, PoolEventHandle {
        ...
        let asset_storage_ref = simple_map::borrow_mut<String, AssetStorage>(&mut storage_ref.asset_storages, &key);
        let share = math64::to_share_roundup(amount, asset_storage_ref.borrowed_amount, asset_storage_ref.borrowed_share);
    ...
    public fun normal_deposited_with(key: String): u64 acquires Storage {
        let storage_ref = borrow_global<Storage>(permission::owner_address());
        normal_deposited_internal(key, storage_ref)
    }
    public fun normal_deposited_internal(key: String, storage: &Storage): u64 {
        if (is_initialized_asset_with_internal(&key, storage)) {
    ...
    public entry fun normal_deposited_share<C>(): u64 acquires Storage {
        let storage_ref = borrow_global<Storage>(permission::owner_address());
        normal_deposited_share_internal(key<C>(), storage_ref)
    }
    public entry fun normal_deposited_share_with(key: String): u64 acquires Storage {
        let storage_ref = borrow_global<Storage>(permission::owner_address());
        normal_deposited_share_internal(key, storage_ref)
    }
    public fun normal_deposited_share_internal(key: String, storage: &Storage): u64 {
        if (is_initialized_asset_with_internal(&key, storage)) {
    ...
    public entry fun test_borrow(owner: &signer, depositor: &signer, borrower: &signer, aptos_framework: &signer) acquires Pool, Storage, PoolEventHandle {
        ...
        // borrow
        let (borrowed, _) = borrow_for<UNI>(borrower_addr, borrower_addr, 100000);
    ...
    public entry fun test_borrow_with_same_as_deposited_amount(owner: &signer, depositor: &signer, borrower: &signer, aptos_framework: &signer) acquires Pool, Storage, PoolEventHandle {
        ...
        // borrow
        let (borrowed, _) = borrow_for<UNI>(borrower_addr, borrower_addr, 1000);
    ...
    public entry fun test_repay(owner: &signer, depositor: &signer, borrower: &signer, aptos_framework: &signer) acquires Pool, Storage, PoolEventHandle {
        ...
        let (repaid_amount, _) = repay<UNI>(borrower, 2000);
    ...
    public entry fun test_repay_with_same_as_total_borrowed_amount(owner: &signer, depositor: &signer, borrower: &signer, aptos_framework: &signer) acquires Pool, Storage, PoolEventHandle {
        ...
        let (repaid_amount, _) = repay<UNI>(borrower, 1005);
    ...
    public entry fun test_repay_more_than_once_sequentially(owner: &signer, depositor: &signer, borrower: &signer, aptos_framework: &signer) acquires Pool, Storage, PoolEventHandle {
        ...
        let (repaid_amount, _) = repay<UNI>(borrower, 1000);
        ...
        let (repaid_amount, _) = repay<UNI>(borrower, 3000);
        ...
        let (repaid_amount, _) = repay<UNI>(borrower, 4000);
