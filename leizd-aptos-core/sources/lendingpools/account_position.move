module leizd::account_position {
    ...
    /// Borrow
    ////////////////////////////////////////////////////
    public(friend) fun borrow<C,P>(account: &signer, borrower_addr: address, amount: u64) acquires Position, AccountPositionEventHandle {
        borrow_internal<C,P>(account, borrower_addr, amount);
    }

    fun borrow_internal<C,P>(account: &signer, borrower_addr: address, amount: u64) acquires Position, AccountPositionEventHandle {
        initialize_if_necessary(account);
    ...
    fun liquidate_internal<C,P>(target_addr: address): (u64,u64,bool) acquires Position, AccountPositionEventHandle {
        if (pool_type::is_type_asset<P>()) {
            ...
            let deposited: u64;
            if (is_collateral_only) {
                assert!(conly_deposited > 0, error::invalid_argument(ENO_DEPOSITED));
                deposited = conly_deposited;
            } else {
                assert!(normal_deposited > 0, error::invalid_argument(ENO_DEPOSITED));
                deposited = normal_deposited;
            };
    ...
    fun rebalance_shadow_internal(addr: address, key1: String, key2: String): (u64,bool,bool) acquires Position, AccountPositionEventHandle {
        let is_collateral_only_C1 = conly_deposited_shadow_with(key1, addr) > 0;
    ...
    fun deposited_volume<P>(addr: address, key: String): u64 acquires Position {
        ...
        deposited_volume_internal(position_ref, key)
    }
    fun deposited_volume_internal<P>(position_ref: &Position<P>, key: String): u64 {
        if (vector::contains<String>(&position_ref.coins, &key)) {
            ...
            if (pool_type::is_type_asset<P>()) {
                total_amount = asset_pool::total_normal_deposited_internal(key);
                total_shares = asset_pool::total_normal_deposited_share_internal(key);
            } else {
                total_amount = (shadow_pool::normal_deposited_with(key) as u128);
                total_shares = (shadow_pool::normal_deposited_share_with(key) as u128);
            };
            let normal_deposited = if (total_amount == 0 && total_shares == 0) balance.deposited_share else math64::to_amount(balance.deposited_share, (total_amount as u64), (total_shares as u64));
            let deposited = normal_deposited + balance.conly_deposited_share; // TODO: convert to amount (may not be necessary in collateral only)
    ...
    fun borrowed_volume<P>(addr: address, key: String): u64 acquires Position {
        let position_ref = borrow_global_mut<Position<P>>(addr);
        borrowed_volume_internal(position_ref, key)
    }
    ...
    public fun deposited_asset<C>(addr: address): u64 acquires Position {
        deposited_asset_with(key<C>(), addr)
    }

    public fun deposited_asset_with(key: String, addr: address): u64 acquires Position {
        if (!exists<Position<AssetToShadow>>(addr)) return 0;
        let position_ref = borrow_global<Position<AssetToShadow>>(addr);
        if (simple_map::contains_key<String,Balance>(&position_ref.balance, &key)) {
            let total_amount = asset_pool::total_normal_deposited_internal(key);
            let total_shares = asset_pool::total_normal_deposited_share_internal(key);
            let user_share = simple_map::borrow<String,Balance>(&position_ref.balance, &key).deposited_share;
    ...
    public fun conly_deposited_asset<C>(addr: address): u64 acquires Position {
        conly_deposited_asset_with(key<C>(), addr)
    }

    public fun conly_deposited_asset_with(key: String, addr: address): u64 acquires Position {
        if (!exists<Position<AssetToShadow>>(addr)) return 0;
        let position_ref = borrow_global<Position<AssetToShadow>>(addr);
        if (simple_map::contains_key<String,Balance>(&position_ref.balance, &key)) {
    ...
    public fun conly_deposited_shadow<C>(addr: address): u64 acquires Position {
        conly_deposited_shadow_with(key<C>(), addr)
    }

    public fun conly_deposited_shadow_with(key: String, addr: address): u64 acquires Position {
        if (!exists<Position<ShadowToAsset>>(addr)) return 0;
        let position_ref = borrow_global<Position<ShadowToAsset>>(addr);
        if (simple_map::contains_key<String,Balance>(&position_ref.balance, &key)) {
            let total_amount = shadow_pool::normal_deposited_with(key); // TODO: use for collateral only
    ...
    public entry fun test_repay_shadow_with_rebalance_all(owner: &signer, account: &signer) acquires Position, AccountPositionEventHandle {
        ...
        assert!(borrowed_shadow<UNI>(account_addr) == 0, 0);
    }
    #[test(owner=@leizd,account=@0x111)]
    ...
    public entry fun test_rebalance_shadow_with_patterns_collateral_only_or_borrowable(owner: &signer, account1: &signer, account2: &signer) acquires Position, AccountPositionEventHandle {
        ...
        assert!(conly_deposited_shadow<UNI>(account1_addr) == 0, 0);
