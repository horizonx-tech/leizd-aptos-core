...
module leizd_aptos_entry::edge {
    ...
    fun test_deposit_asset_with_u64_max_except_interest_rate(owner: &signer, aptos_framework: &signer) {
        // prepares
    ...
    #[test(owner = @leizd_aptos_entry, aptos_framework = @aptos_framework)]
    fun test_deposit_shadow_with_u64_max_except_interest_rate(owner: &signer, aptos_framework: &signer) {
        // prepares
        scenario::initialize_scenario(owner, aptos_framework);
        let signers = scenario::initialize_signer_for_test(2);
        let (account1, account1_addr) = scenario::borrow_account(&signers, 0);
    ...
    #[test(owner = @leizd_aptos_entry, aptos_framework = @aptos_framework)]
    fun test_withdraw_shadow_with_u64_max_except_interest_rate(owner: &signer, aptos_framework: &signer) {
        // prepares
        scenario::initialize_scenario(owner, aptos_framework);
        let signers = scenario::initialize_signer_for_test(2);
        let (account1, account1_addr) = scenario::borrow_account(&signers, 0);
